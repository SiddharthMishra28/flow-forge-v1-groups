...
public interface FlowExecutionRepository extends JpaRepository<FlowExecution, UUID> {
    ...
    @Query("SELECT fe FROM FlowExecution fe LEFT JOIN FlowGroup fg ON fe.flowGroupId = fg.id WHERE " +
           "(:executionId IS NULL OR fe.id = :executionId) AND " +
           "(:flowId IS NULL OR fe.flowId = :flowId) AND " +
           "(:flowGroupName IS NULL OR LOWER(fg.flowGroupName) LIKE LOWER(CONCAT('%', :flowGroupName, '%'))) AND " +
           "(:flowGroupId IS NULL OR fe.flowGroupId = :flowGroupId) AND " +
           "(:iteration IS NULL OR fe.iteration = :iteration) AND " +
           "(:fromDate IS NULL OR fe.createdAt >= :fromDate) AND " +
           "(:toDate IS NULL OR fe.createdAt <= :toDate)")
    Page<FlowExecution> searchAdvanced(@Param("executionId") UUID executionId,
